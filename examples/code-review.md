...
> **指令**：审查一段 FastAPI 代码的安全性，输出问题清单和修复建议
...
**指令 ID**：`JJC-20260221-007`
**指令时间**：2026-02-21 14:30:00
...
| # | 子任务 | 执行节点 | 说明 |
|---|--------|----------|------|
| 1 | 代码安全扫描 | ⚔️ 维控 | 逐文件审查，标记安全问题 |
| 2 | 合规与最佳实践检查 | ⚖️ 探针 | 对照 OWASP Top 10 检查合规性 |
| 3 | 修复方案输出 | ⚔️ 维控 | 针对每个问题给出修复代码 |
...
**棱镜意见：** *"方案合理。维控负责工程侧审查，探针负责合规侧审查，分工明确。注意探针需出具 OWASP 逐项对照表，不能只写'基本合规'。通过校核。"*
...
**探针结论：** 6/10 项需要处理，其中 2 项不合规（A02、A03），需立即修复。
...
**任务档案 ID**：`JJC-20260221-007`
**总耗时**：14 分钟（规划 1 + 校核 0.5 + 执行 12.5）
**总 Token**：8,200
...
棱镜首轮即通过校核，维控和探针分工明确：维控负责代码层面扫描和修复，探针负责合规框架对照。
