...
def test_core_chain_states_exist():
    """本地核心链路状态必须完整存在。"""
    pg = _load_pg_transitions()

    for state in ("Yunxiao", "Xingshu", "Lengjing", "Assigned", "Doing", "Review"):
        assert state in pg, f"{state} missing from task.py STATE_TRANSITIONS"
