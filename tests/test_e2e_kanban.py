...
def test_pure_path_rejected():
    cmd_create('JJC-TEST-E2E-02', '/Users/bingsen/clawd/edict/', 'Xingshu', '星枢', '星枢')
    assert _get_task('JJC-TEST-E2E-02') is None, "纯路径标题应被拒绝"


# ── TEST 3: 正常标题正常创建
def test_normal_title():
    cmd_create('JJC-TEST-E2E-03', '调研工业数据分析大模型应用方案', 'Xingshu', '星枢', '星枢', '云霄整理指令')
...
def test_flow_remark_cleaned():
    cmd_create('JJC-TEST-E2E-04', '调研工业数据分析大模型应用方案', 'Xingshu', '星枢', '星枢')
    cmd_flow('JJC-TEST-E2E-04', '云霄', '星枢', '指令传达：审查/Users/bingsen/clawd/xxx项目 Conversation blah')
...
def test_short_title_rejected():
    cmd_create('JJC-TEST-E2E-05', '好的', 'Xingshu', '星枢', '星枢')
    assert _get_task('JJC-TEST-E2E-05') is None, "短标题应被拒绝"


# ── TEST 6: 传旨前缀剥离
def test_prefix_stripped():
    cmd_create('JJC-TEST-E2E-06', '传旨：帮我写技术博客文章关于智能体架构', 'Xingshu', '星枢', '星枢')
...
def test_state_update():
    cmd_create('JJC-TEST-E2E-07', '测试状态更新与组织联动功能', 'Xingshu', '星枢', '星枢')
    cmd_state('JJC-TEST-E2E-07', 'Lengjing', '方案提交棱镜审议')
    t = _get_task('JJC-TEST-E2E-07')
    assert t is not None
    assert t['state'] == 'Lengjing', f"state应为Lengjing: {t['state']}"
    assert t['org'] == '棱镜', f"org应为棱镜: {t['org']}"


# ── TEST 8: done 完成
def test_done():
    cmd_create('JJC-TEST-E2E-08', '测试任务完成状态标记功能', 'Xingshu', '星枢', '星枢')
...
def test_done_not_overwritable():
    cmd_create('JJC-TEST-E2E-09', '测试已完成任务不可覆盖保护', 'Xingshu', '星枢', '星枢')
    cmd_done('JJC-TEST-E2E-09', '/tmp/output.md', '任务已完成')
    cmd_create('JJC-TEST-E2E-09', '试图覆盖已完成的任务标题', 'Xingshu', '星枢', '星枢')
