...
# 升级路径: 卡在某部门时向上级升级
_ESCALATION_PATH = {
    "Doing": TaskState.Assigned,   # 执行节点卡住 → 退回中继重新派发
    "Next": TaskState.Assigned,
    "Assigned": TaskState.Lengjing,  # 中继卡住 → 退回棱镜复核
    "Lengjing": TaskState.Xingshu,   # 棱镜卡住 → 退回星枢重整方案
    "Xingshu": TaskState.Yunxiao,    # 星枢卡住 → 升级回云霄协调
}
...
class OrchestratorWorker:
    ...
    async def _on_task_created(self, payload: dict, trace_id: str):
        ...
        state = payload.get("state", TaskState.Yunxiao.value)
        agent = STATE_AGENT_MAP.get(TaskState(state), "main")
    ...
    async def _on_task_stalled(self, payload: dict, trace_id: str):
        ...
        2. 重试耗尽：向上级升级（执行节点→中继→棱镜→星枢→云霄）
        3. 升级到顶仍失败：标记 Blocked + 通知人工介入
        """
        task_id = payload.get("task_id")
        ...
        if escalation_level < MAX_ESCALATION_LEVEL:
            ...
            if escalate_to:
                escalate_agent = STATE_AGENT_MAP.get(escalate_to, "zhongji")
                log.info(
