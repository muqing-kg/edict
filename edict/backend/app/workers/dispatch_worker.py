...
def _build_soul_context(agent_id: str) -> str:
    """拼装三层 prompt 层级：GLOBAL.md → groups/*.md → {agent}/SOUL.md。"""
...
def _build_reminder(agent_id: str, payload: dict) -> str:
    ...
    if state == "Doing":
        ...
        reminders.append("棱镜校核：通过则流转 Assigned，不通过则退回 Xingshu 并说明原因。")
...
class DispatchWorker:
    ...
    # 快/慢 Agent 分桶 — 互不阻塞
    _BUCKET_CONFIG = {
        "fast": {"agents": {"main", "xingshu", "lengjing", "zhongji", "tianyan"}, "limit": 4},
        "slow": {"agents": {"yuanliu", "wenshu", "weikong", "tanzhen", "jiwu", "xulie"}, "limit": 3},
    }
